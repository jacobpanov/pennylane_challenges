...
def W(params):
    ...
    # Put your code here
    qubits, parameters = np.shape(params)
    
    for i in range(qubits):
        theta = params[i]
        for j in range(parameters):
            qml.RY(theta[j], wires=j)
        for j in range(parameters - 1):
            qml.CNOT(wires=[j, j+1])
        qml.CNOT(wires=[parameters-1, 0])
...
def quantum_model(param_set, g, x):
    ...
    # Put your code here
    depth, qubits, parameters = np.shape(param_set)
